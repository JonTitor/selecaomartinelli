...
func EfetuarMicracao() error {
	...
	m := gormigrate.New(config.DB, options, []*gormigrate.Migration{
		...
		},
		{
			...
			Migrate: func(tx *gorm.DB) error {
				err := tx.Exec(`INSERT INTO s010pais (codpais, nompais, nompot, estsig, estban) VALUES
				...
				(257, 'Sint Maarten (Dutch part)', 'São Martinho (Países Baixos)', 'SX', 3606);`).Error
				if err != nil {
					return err
				}
				return nil
	...
	m.InitSchema(func(tx *gorm.DB) error {
		...
		referencias := []struct {
			...
			{&modelos.Usuario{}, "codcid", "s030cid (codest)"},
		}
